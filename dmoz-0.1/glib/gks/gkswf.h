...
public:
	...
	PGksBitmap BitmapFromFile(const TStr& FNm) { 
		// TStr FNm => WCHAR *filename
		const char* FNmCStr = FNm.CStr();
		WCHAR *filename = TStr2WCHAR(FNmCStr);
...
private:
	...
	void SaveBmpGks(const TStr& EncoderType, const TStr& FNm);
public:
	TWfBmpGks(const int& Width, const int& Height);
	~TWfBmpGks();

	PGks GetGks() { return Gks; }

	void SavePng(const TStr& FNm) { SaveBmpGks("image/png", FNm); }
	void SaveJpg(const TStr& FNm) { SaveBmpGks("image/jpeg", FNm); }
	void SaveBmp(const TStr& FNm) { SaveBmpGks("image/bmp", FNm); }
	void SaveTiff(const TStr& FNm) { SaveBmpGks("image/tiff", FNm); }
};
