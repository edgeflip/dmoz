...
protected:
    ...
    TStr GetLc(const TStr& Str) { return LcP ? ChDef.GetLcStr(Str) : Str; }
    ...
    const TIntV& GetNGram(const int& NGramId) { return NGramH.GetKey(NGramId); }
    int GetNGramLen(const int& NGramId) { return NGramH.GetKey(NGramId).Len(); }

public:
    TEvalScore(const bool& _LcP = true): LcP(_LcP) { } 
    virtual double Eval(const PTransCorpus& TransCorpus, 
        const TIntV& SentIdV = TIntV()) = 0;
    double EvalSig(const PTransCorpus& FirstTransCorpus, 
        const PTransCorpus& SecondTransCorpus, 
        const int& Shuffles, const int& Seed = 1);
};
...
private:
    TInt MxNGramLen;
public:
    TEvalScoreBleu(const int& _MxNGramLen, const bool& _LcP): 
      TEvalScore(_LcP), MxNGramLen(_MxNGramLen) { }
    double Eval(const PTransCorpus& TransCorpus, const TIntV& _SentIdV = TIntV());
};

/////////////////////////////////////////////////
// NIST-score
class TEvalScoreNist: public TEvalScore {
private:
    TInt MxNGramLen;

public:
    TEvalScoreNist(const int& _MxNGramLen, const bool& _LcP): 
      TEvalScore(_LcP), MxNGramLen(_MxNGramLen) { }
    double Eval(const PTransCorpus& TransCorpus, const TIntV& _SentIdV = TIntV());
};
...
public:
    ...
    // shuffle
    static void Shuffle(const PTransCorpus& InFirstTransCorpus, 
    ...
    void SaveSgm(const TStr& OutOrgFNm, const TStr& OutTransFNm, 
        const TStr& OutRefTransFNm);
