...
PBowMd TLogReg::CalcLogReg(PBowDocBs BowDocBs, PBowDocWgtBs BowDocWgtBs, const TIntV& TrainDIdV, 
		const double& Treshold, const double& ChangeEps, const int& MaxStep, const int& Verbosity) {
    ...
    for (int CatN = 0; CatN < Cats; CatN++) {
        ...
        BowMdV.Add(CalcLogReg(BowDocBs, BowDocWgtBs, CatNm, 
			TrainDIdV, Treshold, ChangeEps, MaxStep, Verbosity));
    } printf("\n");
...
void TBowLogRegMd::GetCfy(const PBowSpV& QueryBowSpV, TFltStrPrV& WgtCatNmPrV) {
    ...
    WgtCatNmPrV.Clr();
    //if (mu >= Treshold) 
	WgtCatNmPrV.Add(TFltStrPr(mu, CatNm));
}
