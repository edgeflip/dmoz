...
public:
    ...
    // learns models from BOW for all categories
    static PBowMd CalcLogReg(PBowDocBs BowDocBs, PBowDocWgtBs BowDocWgtBs, const TIntV& DIdV, 
        const double& Treshold, const double& ChangeEps = 0.01, const int& MaxStep = 200, const int& Verbosity = 0);
};
...
public:
    ...
    // model creation
    static PBowMd New(PBowDocBs BowDocBs, PBowDocWgtBs BowDocWgtBs, const TStr& _CatNm, 
		const TIntV& _DIdV, const double& _Treshold = 0.8, const double& ChangeEps = 0.01, 
		const int& MaxStep = 100, const int& Verbosity = 0) {
			return TLogReg::CalcLogReg(BowDocBs, BowDocWgtBs, _CatNm, 
				_DIdV, _Treshold, ChangeEps, MaxStep, Verbosity); }
    static PBowMd New(PBowDocBs BowDocBs, PBowDocWgtBs BowDocWgtBs,	const TIntV& _DIdV, 
		const double& _Treshold = 0.8, const double& ChangeEps = 0.01, 
		const int& MaxStep = 100, const int& Verbosity = 0) {
			return TLogReg::CalcLogReg(BowDocBs, BowDocWgtBs, _DIdV, 
				_Treshold, ChangeEps, MaxStep, Verbosity); }
