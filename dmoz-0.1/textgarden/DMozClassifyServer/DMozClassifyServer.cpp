...
public:
  TDMozCfyWebSrv(
   ...
   const TStr& LogTxtFNm, const TStr& LogXmlFNm, const int& WebSrvPortN);
  static PWebSrv New(
   const TStr& BowFNm, const TStr& BowPartFNm,
   const TStr& LogTxtFNm, const TStr& LogXmlFNm, const int& WebSrvPortN){
    return PWebSrv(new TDMozCfyWebSrv(BowFNm, BowPartFNm,
     LogTxtFNm, LogXmlFNm, WebSrvPortN));}

  void OnHttpRq(const uint64& SockId, const PHttpRq& HttpRq);
...
TDMozCfyWebSrv::TDMozCfyWebSrv(
 ...
 const TStr& LogTxtFNm, const TStr& LogXmlFNm, const int& WebSrvPortN):
  ...
  TxtLogOut(), fTxtLogOut(NULL), XmlLogOut(), fXmlLogOut(NULL){
  // load bow data
...
void TDMozCfyWebSrv::OnHttpRq(const uint64& SockId, const PHttpRq& HttpRq){
  // log fields
 ...
 const int& MxCats, const double& KWordTsh, const bool& CtxP, const bool& OutXmlFormatP){
  ...
  if (CtxP){
    ...
    /*if ((!Url.Empty())&&(Url->IsOk(usHttp))){
      ...
      PRSet RSet=TGg::WebSearchExternal(GgQueryStr, 50);
      // compose document from link-query to google
      ...
      // compose document form related-query to google
...
int main(int argc, char* argv[]){
  ...
  PWebSrv DMozCfyWebSrv=TDMozCfyWebSrv::New(
   InBowFNm, InBowPartFNm, OutLogTxtFNm, OutLogXmlFNm, WebSrvPortN);
  TLoop::Run();
