...
DLLExport void StrV_Sort(int StrVId, bool Asc);
 ...
 * vector in a vector space spanned by all the words from the corpus.
 * The document's sparse vectors are stored in so called sparse 
 * 'term-documet matrix', where one column corresponds to one document's 
 * sparse vector.</p>
 *
...
/**
 * <p>Loads SemSpace object from a file into memory and returns its handle.</p>
 * @param Input SemSpace file name
 * @return handle to loaded SemSpace object
 */
DLLExport int SemSpace_LoadFromFile(char* FNm);
/**
 * <p>Saves the given SemSpace object from the memory into a file.</p>
 * @param handle for the SemSpace object
 * @param output SemSpace file name
 * @return true if saving successful
 */
DLLExport void SemSpace_SaveToFile(int SemSpaceId, char* FNm);
/**
 * <p>Saves statistics of the given SemSpace object to a text file.</p>
 * @param handle for the SemSpace object
 * @param handle for the BowDocBs object on which SemSpace is based
 * @param output SemSpace statistic file name
 * @return true if saving successful
 */
DLLExport void SemSpace_SaveStatToFile(int SemSpaceId, int BowDocBsId, char* FNm);
/**
 * <p>Deletes the given SemSpace object from the memory.</p>
 * @param handle for the SemSpace object
 * @return true if successfuly deleted
 */
...
/**
 * <p>Returns the dimensionality of the semantic space.</p>
 * @param handle for the SemSpace object
 * @return number of dimensions
 */
DLLExport int SemSpace_GetDims(int SemSpaceId);
/**
 * <p>Returns a handle to the vector of the (weight, word) pairs
 * of the basis vector for the given dimension. The elements of vector 
 * are sorted with the starting with the highest weights.</p>
 *
 * <p>Semantic space is a subspace in bag-of-words space. It is defined
 * by a set of basis vector where each basis vector splits the space in 
 * one or two semantic meanings. Since the basis vectors also live in the
 * bag-of-words space they can be described by the keywords which
 * correspod to the elements in vectors with the highest absolute value.</p>
 *
 * @param handle for the SemSpace object
 * @param handle for the BowDocBs object on which SemSpace is based
...
/**
 * <p>Generates LSI semantic space from the documents in the given
 * BowDocBs object. Latent Semantic Indexing (LSI) is a technique
 * which discovers weighted sets of words which frequently co-occure 
 * in the coprus. It is based on Singular Value Decomposition (SVD)
 * of term-document matrix which is stored in the BowDocBs object.</p>
 *
 * <p>Semantic space is a subspace in bag-of-words space. It is defined
 * by a set of basis vector where each basis vector splits the space in 
 * one or two semantic meanings. Since the basis vectors also live in the
 ...
 * correspod to the elements in vectors with the highest absolute value.</p>
 *
 * @param handle for the BowDocBs object on which SemSpace should be based
 * @param required number of dimensions
 * @return handle to created SemSpace object
 */
DLLExport int SemSpace_GetLSI(int BowDocBsId, int SpaceDims);
/**
 * <p>Generates LSI semantic space from the centroids in the given
 * BowDocPart object. Latent Semantic Indexing (LSI) is a technique
 * which discovers weighted sets of words which frequently co-occure 
 * in the coprus. It is based on Singular Value Decomposition (SVD)
 * of term-centroid matrix which is stored in the BowDocPart object.</p>
 *
 * <p>Semantic space is a subspace in bag-of-words space. It is defined
 * by a set of basis vector where each basis vector splits the space in 
 * one or two semantic meanings. Since the basis vectors also live in the
...
/**
 * <p>Generates PCA semantic space from the documents in the given
 * BowDocBs object. Principal Component Analysis (PCA) is a technique
 * which discovers directions of maximal variance along the documents
 * from the coprus. The directions are top eigne vectors of term
 * covariance matrix which is obtained from the documents based
 * on term co-occurance within document.</p>
 *
 * <p>Semantic space is a subspace in bag-of-words space. It is defined
 * by a set of basis vector where each basis vector splits the space in 
 * one or two semantic meanings. Since the basis vectors also live in the
...
///**
// * <p>Loads VizMap object from a file into memory and returns its handle.</p>
// * @param Input VizMap file name
// * @return handle to loaded VizMap object
// */
//DLLExport int VizMap_LoadFromFile(char* FNm);
///**
// * <p>Saves the given VizMap object from the memory into a file.</p>
// * @param handle for the VizMap object
// * @param output VizMap file name
// * @return true if saving successful
// */
//DLLExport void VizMap_SaveToFile(int VizMapId, char* FNm);
///**
// * <p>Deletes the given VizMap object from the memory.</p>
// * @param handle for the VizMap object
// * @return true if successfuly deleted
// */
...
// Visualization-Map
///**
// * <p>Generates a new visualization map of documents from BowDocBs object
// * using clustering, semantic space and multi-dimensional scaling. Each document 
// * is prepresented with a two-dimensional point so that the documents more 
// * similar to each other are positioned closer than non-similar documents. 
// * This method first postions cluster centroids and than interpolates the 
// * position of each document based on its similarity to the centroids. The 
// * document positions are stored in the VizMap object.</p>
// *
// * <p>The process is composed of three steps. We start with bag-of-words 
// * representation where each cluster centroid is represented with 
// * some high-dimensional vector. Provided SemSpace object is used for
// * the first linear reduction to a hunderd or so dimension. The exact number
// * of dimensions used from SemSpace is controled with SemSpaceTsh 
// * parameter. The second step is done with Multi Dimensional Scaling (MDS)
// * non-linear dimensionality reduction method which positions the centroids 
// * on two dimensional plane so that their Euclidian distance matches their 
// * distance in the semantic space as much as possible. Finally the document 
// * positions are interpolated based on the document's similarity to centroids.</p>
// *
// * <p>Number of dimensions from semantic space which are used in the first step
// * is the smallest number of main dimension so that average approximation of 
// * centroids using this set of dimensions is above SemSpaceTsh parameter. If
// * there dimensionality of semantic space is to low to achive such approximation
// * than all dimensions are used. Approximation is massured are ratio between
// * the norm of the approximated vector and original vector.</p>
// * 
// * <p>MDS algorithm has several stoping criterias: number of iterations,
// * runing time and difference of calculated positions between iterations. The 
// * algorithms stops when at least one of the is reached.</p>
// *
// * @param handle for the BowDocBs object, documents from it will be visualized
// * @param handle for the BowDocWgtBs object with weighted document vectors
// * @param handle for the BowDocPart object with clusters generted from the corpus
// * @param handle for the SemSpace object based on corpus
// * @param number of centroids used for interpolating documents position (usually 3)
// * @param average approximation threshold (usualy 0.7)
// * @param maximal number of iterations that MDS algorithm can do (usualy 10000)
// * @param maximal number of seconds that MDS algorithm can use (usualy 100)
// * @param minimal difference of positioning between iterations (usualy 0.0001)
// * @return handle to created VizMap object
// */
...
///**
// * <p>Generates a new visualization map of documents from BowDocBs object
// * using semantic space and multi-dimensional scaling. Each document is 
// * prepresented with a two-dimensional point so that the documents more similar 
// * to each other are positioned closer than non-similar documents. The document 
// * positions are stored in the VizMap object.</p>
// *
// * <p>The process is composed of two steps. We start with bag-of-words 
// * document representation where each document is represented with 
// * some high-dimensional vector. Provided SemSpace object is used for
// * the first linear reduction to a hunderd or so dimension. The exact number
// * of dimensions used from SemSpace is controled with SemSpaceTsh 
// * parameter. The second step is done with Multi Dimensional Scaling (MDS)
// * non-linear dimensionality reduction method which positions the documents 
// * on two dimensional plane so that their Euclidian distance matches their 
// * distance in the semantic space as much as possible.</p>
// *
// * <p>Number of dimensions from semantic space which are used in the first step
// * is the smallest number of main dimension so that average approximation of 
// * documents using this set of dimensions is above SemSpaceTsh parameter. If
// * there dimensionality of semantic space is to low to achive such approximation
// * than all dimensions are used. Approximation is massured are ratio between
// * the norm of the approximated vector and original vector.</p>
// * 
...
//
///**
// * <p>Returns the handle to BowDocBs object which is visualized with given 
// * VizMap object.</p>
// * @param handle for the VizMap object
// * @return handle for the BowDocBs object
// */
//DLLExport int VizMap_GetBowDocBsId(int VizMapId);
///**
// * <p>Returns the handle to BowDocWgtBs object which is visualized with given 
// * VizMap object.</p>
// * @param handle for the VizMap object
// * @return handle for the BowDocWgtBs object
// */
...
///**
// * <p>Returns the number of documents that are visualized with given VizMap object.</p>
// * @param handle for the VizMap object
// * @return number of documents
// */
//DLLExport int VizMap_GetDocs(int VizMapId);
///**
// * <p>Returns the document ID from the VizMap object's document list that.</p>
// * @param handle for the VizMap object
// * @param position of document in the VizMap object's document list
// * @return documents ID
// */
//DLLExport int VizMap_GetDId(int VizMapId, int DIdN);
///**
// * <p>Returns the X coordinate for the given document.</p>
// * @param handle for the VizMap object
// * @param position of document in the VizMap object's document list
// * @return X coordinate
// */
...
///**
// * <p>Returns the Y coordinate for the given document.</p>
// * @param handle for the VizMap object
// * @param position of document in the VizMap object's document list
// * @return Y coordinate
// */
...
///**
// * <p>Returns the document ID of a document that is the closest to the
// * given point based on Euclidian distance.</p>
// * @param handle for the VizMap object
// * @param point's coordinate X
// * @param point's coordinate Y
// * @param maximal distance to the point that is still valid
// * @return document ID, -1 if document is close enough to the point
// */
//DLLExport int VizMap_GetXYDId(int VizMapId, double X, double Y, double Radius=0.01);
//
///**
// * <p>Generates landscapes which reflects the density of the documents on
// * the two-dimensional plane.</p>
// * @param handle for the VizMap object
// * @param number of landscapes that should be calculated
// * @param width resolution of landscape
// * @param height resolution of landscape
// * @param influence of one document on the density (usually 0.01)
// * @param decay factor for BellSigma that is applied to it for subsequent landscapes
// */
...
///**
// * <p>Number of landscapes stored in the VizMap objects.</p>
// * @param handle for the VizMap object
// * @return number of landscapes
// */
//DLLExport int VizMap_GetLandscapes(int VizMapId);
///**
// * <p>Saves the visualization as VRML file. Documents and keywords are positioned
// * on top of the density landscape.</p>
// * @param handle for the VizMap object
// * @param output VRML file name
// * @param number of the landscape that should be used in VRML
// * @param if true than document names are shown
// * @param if true than document points are shown
// * @param if true than landscape keywords are shown
// * @param font size (usually 1.0)
// */
...
//
///**
// * <p>Generates a list of 1000 keywords for randomly chosen locations on the two 
// * dimensional plane from VizMap object. First a random position for the keyword is 
// * selected. Than the centroid vector from all the bag-of-words vectors of near-by 
// * documents is calculated (near-by is defined by Radius parameter). A set of top 
// * keywords is extracted from the centroid vector (size of the set is defined by 
// * Candidates parameter) and the keyword assigned to this position is randomly 
// * selected from this list.</p>
// *
// * @param handle for the VizMap object
// * @param number of keywords that should be calculated
// * @param radius defining the area for which centroid vector is calculates
// * @param number of top candidates from which the keywords are selected
// */
//DLLExport void VizMap_GenKeywords(int VizMapId,
// int KWords=1000, double Radius=0.2, int Candidates=3);
///**
// * <p>Returns the number of keywords that are stored in the VizMap object.</p>
// * @param handle for the VizMap object
// * @return number of keywords
// */
//DLLExport int VizMap_GetKWords(int VizMapId);
///**
// * <p>Returns the word string for the given keyword.</p>
// * @param handle for the VizMap object
// * @param location of the keyword in the keyword list
// * @return word string
// */
//DLLExport char* VizMap_GetKWordStr(int VizMapId, int KWordN);
///**
// * <p>Returns the keyword's X coordinate.</p>
// * @param handle for the VizMap object
// * @param location of the keyword in the keyword list
// * @return X coordinate
// */
...
///**
// * <p>Returns the keyword's Y coordinate.</p>
// * @param handle for the VizMap object
// * @param location of the keyword in the keyword list
// * @return Y coordinate
// */
...
///**
// * <p>Returns the top keywords from the centroid calculated for the
// * area around given point.</p>
// * @param handle for the VizMap object
// * @param point's coordinate X
// * @param point's coordinate Y
// * @param radius defining the area around the point for which centroid vector is calculates
// * @param number of top keywords that should be returned
// * @return String-Vector handle
// */
//DLLExport int VizMap_GetKWordStrV(int VizMapId, 
// double X, double Y, double Radius, int MxKWords);
